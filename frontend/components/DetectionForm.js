...
export default function DetectionForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (err.response) {
        const errorMessage =
          err.response.data?.error || `Error server: ${err.response.status}`;
  ...
  const renderResults = () => {
    ...
    return (
      <div className="backdrop-blur-sm bg-white/90 dark:bg-gray-800/90 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 mt-8 overflow-hidden transition-all duration-300 ease-in-out">
        <div
          className={`py-3 px-4 ${colors.bg} ${colors.border} border-b flex items-center`}
        >
          ...
          <div
            className={`${colors.text} font-semibold px-3 py-1 rounded-full bg-white/80 dark:bg-black/20 text-sm`}
          >
        ...
        <div className="p-6 flex flex-col md:flex-row gap-8">
          <div className="flex-1 space-y-6">
            <div>
              <p className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium mb-1">
                Klasifikasi
              </p>
              <div
                className={`rounded-lg ${colors.bg} ${colors.border} border p-4 flex items-center`}
              >
                <div
                  className={`w-3 h-3 rounded-full mr-3 ${colors.text.replace(
                    "text",
                    "bg"
                  )}`}
                ></div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative pt-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    className={`text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full ${colors.bg} ${colors.text}`}
                  >
                    {confidencePercent}%
                  </div>
                </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
                Hasil analisis menunjukkan tingkat retinopati{" "}
                <strong className={colors.text}>{diagnosisClass}</strong>.
                Disarankan untuk berkonsultasi dengan dokter spesialis mata
                untuk tindak lanjut.
              </p>
            </div>
          </div>
          ...
          {previewUrl && (
            <div className="md:w-2/5 flex flex-col">
              <p className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium mb-2">
                Gambar Diunggah
              </p>
    ...
    <div className="max-w-4xl mx-auto">
      <div className="backdrop-blur-sm bg-white/90 dark:bg-gray-800/90 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="border-b border-gray-100 dark:border-gray-700 py-4 px-6 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Format JPG atau PNG untuk hasil terbaik
          </p>
        </div>
